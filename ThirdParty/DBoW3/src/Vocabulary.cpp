...
void Vocabulary::load_frombin(const std::string &filename)throw(std::runtime_error)
{
    std::ifstream f;
    f.open(filename.c_str());

    if(f.eof()) {
      throw std::runtime_error("Vocabulary:: load_frombin  Could not open file for reading:"+filename);
    }

    if(f.fail()) {
      throw std::runtime_error( "Vocabulary loading failure: This is not a correct binary file!" );
    }
   ...
   // while(!f.eof())
    while((!f.eof()) && ( m_nodes.size()<(unsigned int)expected_nodes) )
    {
        ...
        nIsLeaf=nIsLeafuc;
        unsigned char array[32];                // the number of element is stored in 32
        f.read((char*)array,(long)32);
        m_nodes[nid].descriptor.create(1, 32, CV_8U);
        fromArray8U(m_nodes[nid].descriptor,(unsigned char *)array);
