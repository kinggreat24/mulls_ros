...
public:
  ...
  */
  void load_frombin(const std::string &filename)throw(std::runtime_error);
...
protected:
  ...
  inline void toArray8U(const cv::Mat &descriptors, unsigned char * array)
  {
    const unsigned char *d = descriptors.ptr<unsigned char>();
    std::copy(d, d+32, array);
  }

  // --------------------------------------------------------------------------

  inline void fromArray8U(cv::Mat &descriptors, unsigned char * array)
  {
    unsigned char *d = descriptors.ptr<unsigned char>();
    std::copy(array, array+32, d);
  }
