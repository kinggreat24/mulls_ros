...
public:
	RegistrationParams(const float &scanPeriod_ = 0.1,
					   ...
					   const float &lessFlatFilterSize_ = 0.4,
...
public:
    ...
    */
	void processScanlines(const Time &scanTime, std::vector<pcl::PointCloud<Point_T>> const &laserCloudScans);
    ...
    */
	void projectPointToStartOfSweep(Point_T &point, float relTime);
...
private:
     ...
     */
	void transformToStartIMU(Point_T &point);
...
private:
	...
	pcl::PointCloud<Point_T> _laserCloud; ///< full resolution input cloud
	std::vector<IndexRange> _scanIndices;		 ///< start and end indices of the individual scans withing the full resolution cloud

	pcl::PointCloud<Point_T> _cornerPointsSharp;		///< sharp corner points cloud
	pcl::PointCloud<Point_T> _cornerPointsLessSharp; ///< less sharp corner points cloud
	pcl::PointCloud<Point_T> _surfacePointsFlat;		///< flat surface points cloud
	pcl::PointCloud<Point_T> _surfacePointsLessFlat; ///< less flat surface points cloud
...
#endif //BASIC_SCAN_REGISTRATION_H