...
#pragma omp parallel for //Multi-thread
		...
		bool pointcloud_depthfilter(
			const typename pcl::PointCloud<PointT>::Ptr &pointcloud_in, typename pcl::PointCloud<PointT>::Ptr &cloud_out,
			const char *field,
			const float minDepth,
			const float maxDepth)
		{
			//条件
			typename pcl::ConditionAnd<PointT>::Ptr range_condition(new pcl::ConditionAnd<PointT>());
			typename pcl::FieldComparison<PointT>::ConstPtr cond_1(new pcl::FieldComparison<PointT>(field, pcl::ComparisonOps::GT, minDepth));
			typename pcl::FieldComparison<PointT>::ConstPtr cond_2(new pcl::FieldComparison<PointT>(field, pcl::ComparisonOps::LT, maxDepth));
			range_condition->addComparison(cond_1);
			range_condition->addComparison(cond_2);

			typename pcl::ConditionalRemoval<PointT> condition;
			condition.setCondition(range_condition);
			condition.setInputCloud(pointcloud_in);
							  ...
							  bool kd_tree_already_built = false, const typename pcl::search::KdTree<PointT>::Ptr &built_tree = NULL) //according to curvature
		{
			...
			std::sort(cloud_in_out->points.begin(), cloud_in_out->points.end(), [](const PointT &a, const PointT &b)
							  ...
							  bool kd_tree_already_built = false, const typename pcl::search::KdTree<PointT>::Ptr &built_tree = NULL) //according to curvature
		{
			...
			std::sort(cloud_in->points.begin(), cloud_in->points.end(), [](const PointT &a, const PointT &b)
					  { return a.normal[3] > b.normal[3]; }); //using the unused normal[3] to represent what we want
