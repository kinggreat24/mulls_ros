...
#pragma omp parallel for //Multi-thread
			...
			else //fixed num correspondence
			{
				...
				std::sort(dist_array.begin(), dist_array.end(), [](const std::pair<int, float> &a, const std::pair<int, float> &b)
...
#if TEASER_ON
			std::cout << "**********      coarse_reg_teaser      **********" << std::endl;
		...
		// interface for the implement of F-LOAM algorithm
		...
		{
			// std::cout << "Begin registration 1" <<std::endl;
			std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
...
#pragma omp section
			...
			// std::cout << "Build Kdtree done in [" << build_kdtree_time.count() * 1000.0 << "] ms" <<std::endl;
...
// std::cout << "Source point cloud updating done in [" << update_time.count() * 1000.0 << "] ms for iteration [" << i << "]" <<std::endl;
...
#pragma omp section
					{
						...
						{
							//Check normal vector
							determine_corres(pc_ground_sc, pc_ground_tc, registration_cons.block1->tree_ground, dis_thre_ground, corrs_ground, normal_shooting_on, true, normal_bearing);
						}
					}
#pragma omp section
					{
						if (used_feature_type[1] == '1' && pc_pillar_sc->size() > 0)
						{
							...
							determine_corres(pc_pillar_sc, pc_pillar_tc, registration_cons.block1->tree_pillar, dis_thre_pillar, corrs_pillar, false, true, normal_bearing);
						}
					}
#pragma omp section
					{
						...
						{
							//Check normal vector
							determine_corres(pc_facade_sc, pc_facade_tc, registration_cons.block1->tree_facade, dis_thre_facade, corrs_facade, normal_shooting_on, true, normal_bearing);
						}
						if (used_feature_type[3] == '1' && pc_beam_sc->size() > 0)
						{
							//Check primary vector
							determine_corres(pc_beam_sc, pc_beam_tc, registration_cons.block1->tree_beam, dis_thre_beam, corrs_beam, false, true, normal_bearing);
						}
					}
				}
				...
				// std::cout << "Used correspondences [G: " << (*corrs_ground).size() << " P: " << (*corrs_pillar).size() << " B: " << (*corrs_beam).size()
				// 		  << " F: " << (*corrs_facade).size() << " R: " << (*corrs_roof).size() << " V: " << (*corrs_vertex).size() << " ]." <<std::endl;
				// std::cout << "Correspondence searching done in [" << corre_search_time.count() * 1000.0 << "] ms for iteration [" << i << "]"<<std::endl;
				...
				// std::cout << "Transformation estimation done in [" << tran_estimation_time.count() * 1000.0 << "] ms for iteration [" << i << "]" <<std::endl;
			...
			// std::cout << "The posterior overlapping ratio is [" << registration_cons.confidence << "]"<<std::endl;

			std::chrono::steady_clock::time_point toc = std::chrono::steady_clock::now();
			std::chrono::duration<double> reg_time_used = std::chrono::duration_cast<std::chrono::duration<double>>(toc - tic);

			// std::cout << "Registration done in [" << reg_time_used.count() * 1000.0 << "] ms."<<std::endl;
			// std::cout << "Final tran. matrix:\n"
			// 		  << registration_cons.Trans1_2<<std::endl;
