...
class DataIo : public CloudUtility<PointT>
{
  public:
    ...
    bool check_overwrite_exsiting_file_or_not(std::string &output_file, std::string new_file_tail = "_new.txt")
    {
        ...
        std::cout << "A new file [" << output_file << "] would be generated and used later"<<std::endl;
    }
    ...
    bool load_calib_mat(const std::string calib_file, Eigen::Matrix4d &calib_mat, bool need_inverse=false)
    {
        ...
        if(need_inverse)
            calib_mat = calib_mat.inverse();

        std::cout << "Calib matrix loaded\n";
