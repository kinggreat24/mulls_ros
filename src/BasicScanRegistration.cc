...
RegistrationParams::RegistrationParams(const float &scanPeriod_,
	...
	: scanPeriod(scanPeriod_),
	  ...
	  maxCornerLessSharp(5 * maxCornerSharp_),
	  maxSurfaceFlat(maxSurfaceFlat_),
	  lessFlatFilterSize(lessFlatFilterSize_),
	  surfaceCurvatureThreshold(surfaceCurvatureThreshold_){};

void BasicScanRegistration::processScanlines(const Time &scanTime, std::vector<pcl::PointCloud<Point_T>> const &laserCloudScans)
{
...
void BasicScanRegistration::projectPointToStartOfSweep(Point_T &point, float relTime)
{
...
void BasicScanRegistration::transformToStartIMU(Point_T &point)
{
...
void BasicScanRegistration::extractFeatures(const uint16_t &beginIdx)
{
	...
	{
		pcl::PointCloud<Point_T>::Ptr surfPointsLessFlatScan(new pcl::PointCloud<Point_T>);
    ...
    for (int j = scanStartIdx; j <= scanEndIdx; j++) {
      _laserCloud[j].intensity = i + _scanPeriod * (j - scanStartIdx) / scanSize;
		...
		pcl::PointCloud<Point_T> surfPointsLessFlatScanDS;
		pcl::VoxelGrid<Point_T> downSizeFilter;
...
void BasicScanRegistration::setScanBuffersFor(const size_t &startIdx, const size_t &endIdx)
{
	...
	{
		const Point_T &previousPoint = (_laserCloud[i - 1]);
		const Point_T &point = (_laserCloud[i]);
		const Point_T &nextPoint = (_laserCloud[i + 1]);
