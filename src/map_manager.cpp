...
namespace lo
{
                                      ...
                                      float dynamic_dist_thre_max, float near_dist_thre, bool recalculate_feature_on)
    {
        ...
        // (1) transform from the last local_map to the target frame
        // 计算当前地图到当前帧的变换矩阵[ tran_target_map ]
        Eigen::Matrix4d tran_target_map = Eigen::Matrix4d::Identity();
        tran_target_map = last_target_cblock->pose_lo.inverse() * local_map->pose_lo;

        // (2) to avoid building kd-tree twice , do the map based filtering at first before the transforming
        // 将当前帧的激光点转到局部地图坐标系中
        ...
        // add by kinggreat24 局部原始点云处理
        //  {
        //      last_target_cblock->pc_local_map_raw_w.reset(new pcT());
        //      last_target_cblock->transform_raw_pointcloud(last_target_cblock->pc_local_map_raw_w, tran_target_map.inverse());

        //     if (!local_map->pc_local_map_raw_w)
        //         local_map->pc_local_map_raw_w.reset(new pcT());
        //     *local_map->pc_local_map_raw_w += *last_target_cblock->pc_local_map_raw_w;

        //     //降采样
        //     local_map->pc_local_map_raw_w_down.reset(new pcT());
        //     cf.voxel_downsample(local_map->pc_local_map_raw_w, local_map->pc_local_map_raw_w_down, 0.5);
        //     *local_map->pc_local_map_raw_w = *local_map->pc_local_map_raw_w_down;
        ...
        // Re-calculation primary vector and filter some of the points in the local map at the same time
        if (recalculate_feature_on)
        {
        ...
        // printf("last_target_cblock pc_raw size: %d\r\n", last_target_cblock->pc_raw->size());
...
#pragma omp section
                                                           ...
                                                           float dynamic_dist_thre_min, float dynamic_dist_thre_max, float near_dist_thre)
    {
        ...
        {
            if (feature_pts->points[i].x * feature_pts->points[i].x + feature_pts->points[i].y * feature_pts->points[i].y >
